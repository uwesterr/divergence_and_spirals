...
PREDEFINED_SCENARIOS = {
    "Default Balanced": {
        "acquisition_margin": 5,
        "uncertainty_cone": 1000,
        "v": 0.31,
        "b": 5.6
    },
    "Short-Range, High-Speed": {
        "acquisition_margin": 3,
        "uncertainty_cone": 700,
        "v": 0.4,
        "b": 7.0
    },
    "Long-Range, Max Sensitivity": {
        "acquisition_margin": 10,
        "uncertainty_cone": 1500,
        "v": 0.2,
        "b": 4.0
    }
}
...
# Function to apply selected scenario
def apply_scenario(scenario_name):
    scenario = PREDEFINED_SCENARIOS[scenario_name]
    st.session_state.acquisition_margin = scenario["acquisition_margin"]
    st.session_state.uncertainty_cone = scenario["uncertainty_cone"]
    st.session_state.v = scenario["v"]
    st.session_state.b = scenario["b"]
    # It's important to call calc() after updating session state if sliders don't auto-trigger
    # However, since sliders have on_change=calc, changing their session_state values should trigger calc.
    # If not, uncomment the next line:
    # calc()

scenario_options = list(PREDEFINED_SCENARIOS.keys())
selected_scenario = st.sidebar.selectbox(
    "Select a Predefined Scenario:",
    options=scenario_options,
    index=0,  # Default to the first scenario
    on_change=apply_scenario,
    args=(st.session_state.get("scenario_selectbox_key", scenario_options[0]),) # Workaround for on_change with selectbox
)
# Store the selection to manage on_change behavior correctly
st.session_state["scenario_selectbox_key"] = selected_scenario


acquisition_margin = st.sidebar.slider("Acquisition margin [dB]:", key="acquisition_margin", on_change=calc, value=st.session_state.get("acquisition_margin", 5), min_value=0, max_value=20, step=1)

st.sidebar.markdown("""
## Input spiral parameters
""")

# Initialize session state for sliders if not already set by a scenario
if "uncertainty_cone" not in st.session_state:
    st.session_state.uncertainty_cone = PREDEFINED_SCENARIOS[selected_scenario]["uncertainty_cone"]
if "v" not in st.session_state:
    st.session_state.v = PREDEFINED_SCENARIOS[selected_scenario]["v"]
if "b" not in st.session_state:
    st.session_state.b = PREDEFINED_SCENARIOS[selected_scenario]["b"]
...
if 'acquisition_margin' not in st.session_state:
    st.session_state['acquisition_margin'] = st.session_state.get("acquisition_margin", PREDEFINED_SCENARIOS[selected_scenario]["acquisition_margin"])
    st.session_state['session_state_acquisition_margin_linear'] = 0 # This will be recalculated
    st.session_state['spiral_arm_distance_speedup'] = 0  # This will be recalculated
    st.session_state['spiral_velocity_speedup'] = 10 # This will be recalculated
    st.session_state['total_spiral_speedup'] = 0 # This will be recalculated
    st.session_state['show_trace'] = 1
    # calc() # Call calc initially if needed, or ensure it's called after scenario load

start_fov = st.sidebar.slider( "UC to scan [µrad]:", key="uncertainty_cone", on_change=calc, value=st.session_state.get("uncertainty_cone", 1000), min_value= 100, max_value= 3000, step=10)

vel = st.sidebar.slider( "Scan velocity [rad/s]:", key="v", on_change=calc, value=st.session_state.get("v", 0.31), min_value= 0.00001, max_value= .5, step=.01)
b = st.sidebar.slider( "Spiral arm distance [µrad]:", key="b", on_change=calc, value=st.session_state.get("b", 5.6), min_value= 3.0, max_value= 50.0, step=.1)

# Ensure calc is called at least once after initial setup or scenario load
if 'total_spiral_speedup' not in st.session_state or st.session_state.total_spiral_speedup == 0: # Check if calc has run
    apply_scenario(selected_scenario) # Apply default or selected scenario
    calc()
